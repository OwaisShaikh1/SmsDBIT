// templates/templates/template_crud.js
...
// Ensure listeners exist when modal opens
...
function getVariables() {
  return [...document.querySelectorAll(".variable-row")].map(row => {
    ...
    const type = row.querySelector(`.var-type-select[data-row-id="${id}"]`)?.value;
...
function updateVariableChips() {
  ...
  // Make chips clickable + draggable
...
function updateCharCount() {
  ...
  document.getElementById("charCount").textContent = `${el.value.length}/1600`;
}
